...
import {
  BillAccountDto,
  createBillAccount,
  createMonthlyBillAccountSeries,
  deleteBillAccount,
  deleteMonthlyBillAccountOccurrence,
  deleteMonthlyBillAccountSeries,
  getBillAccounts,
  getLegacyBillPaymentCandidates,
  hideBillAccountFromList,
  payBillAccount,
  syncCreditCardInvoiceBills,
  syncMonthlyBillAccountOccurrences,
  unpayBillAccount,
  updateBillAccount,
} from "@/lib/services/billAccounts";
...
const getStatusColor = (status: string) => {
  const colors: Record<string, string> = {
    ...
    overdue:
      "border-rose-500/20 bg-rose-500/10 text-rose-700 dark:bg-rose-500/18 dark:text-rose-300",
    "due-soon":
      "border-amber-500/20 bg-amber-500/10 text-amber-700 dark:bg-amber-500/18 dark:text-amber-300",
    pending:
...
const isCreditCardInvoiceBill = (bill: BillAccount): boolean => {
  return (
    bill.source === "credit_card_invoice" ||
...
const getCreditCardInvoiceSummaries = (
  ...
  return cards.flatMap((card) => {
    ...
    const totalsByPeriod = cardItems.reduce<Record<string, number>>(
      (totals, item) => {
        const itemPeriodKey = getInvoicePeriodKeyForDate(
          item.date,
          closingDay,
          dueDay,
        );
        totals[itemPeriodKey] =
          (totals[itemPeriodKey] || 0) + Math.abs(item.amount);
        return totals;
      },
      {},
    );

    return Object.entries(totalsByPeriod).map(([periodKey, totalAmount]) => {
...
export const Main: FC = () => {
  ...
  const [selectedBillForPayment, setSelectedBillForPayment] =
    useState<BillAccount | null>(null);
  ...
  const [periodFilter, setPeriodFilter] = useState<
    "current" | "next" | "all" | "custom"
  >("current");
  const [customMonth, setCustomMonth] = useState<string>("");
  const [showAddTransactionDialog, setShowAddTransactionDialog] =
    useState(false);
  const [creditCardInvoicePeriodKey, setCreditCardInvoicePeriodKey] =
    useState("");
  const [billToDelete, setBillToDelete] = useState<BillAccount | null>(null);
  ...
  const filterBillsByPeriod = (
    bills: BillAccount[],
    period: "current" | "next" | "all" | "custom",
  ...
  const filterBillsByStatus = (bills: BillAccount[], status: string) => {
    ...
    return bills
      .map((bill) => ({ bill, billStatus: getBillStatus(bill) }))
      .filter(({ billStatus }) => {
        if (status === "pending")
          return billStatus === "pending" || billStatus === "due-soon";
  ...
  const getFilteredBills = () => {
    ...
    const byPeriod = filterBillsByPeriod(
      visibleBills,
      periodFilter,
      customMonth,
    );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [billsData, transactionsData, cardsData, cardTransactionsData] =
          await Promise.all([
            getBillAccounts(),
            getTransaction(),
            getUserCreditCards(),
        ...
        const generatedMonthlyBills = await syncMonthlyBillAccountOccurrences();
        const availableBills = [...billsData, ...generatedMonthlyBills];
        const invoiceSummaries = getCreditCardInvoiceSummaries(
          cardsData || [],
          cardTransactionsData || [],
        );
        const hasExistingInvoiceBill = (cardId: string, periodKey: string) => {
          return availableBills.some(
            (bill) =>
              bill.creditCardId === cardId &&
              bill.creditCardInvoicePeriodKey === periodKey,
          );
        };
        const syncedInvoiceBills = await syncCreditCardInvoiceBills(
          invoiceSummaries
            .filter(
              ({ card, periodKey, totalAmount, isPaid }) =>
                totalAmount > 0 &&
                (!isPaid || hasExistingInvoiceBill(card.id, periodKey)),
            )
            .map(
              ({
                card,
        ...
        availableBills.forEach((bill) => billsById.set(bill.id, bill));
  ...
  const calculateProjection = () => {
    ...
    const totalBalance = currentMonthTransactions.reduce(
      (acc, t) => acc + t.amount,
      0,
    );
  ...
  const { totalBalance, totalPending, projectedBalance } =
    calculateProjection();
  ...
  const getCardsWithOpenInvoices = () => {
    ...
    return getCreditCardInvoiceSummaries(userCards, cardTransactions)
      ...
      .filter((_, index, invoices) => {
        const firstFutureInvoiceIndex = invoices.findIndex(
          (invoice) => invoice.dueDate >= today,
        );
        return (
          firstFutureInvoiceIndex === -1 || index >= firstFutureInvoiceIndex
        );
      });
  };

  const handleSelectCardSuggestion = (
    card: UserCreditCard,
    invoiceAmount: number,
    periodKey: string,
  ...
  const handleAddBill = async () => {
    ...
    try {
      if (recurrence === "monthly") {
        const newBill = await createMonthlyBillAccountSeries({
          description,
          amount,
          dueDate,
        });
        const generatedBills = await syncMonthlyBillAccountOccurrences();
        setBills((prev) =>
          sortBillsByDate([
      ...
      } else if (recurrence === "installments" && installments > 1) {
        ...
        const newBill =
          creditCardId && creditCardInvoicePeriodKey
            ? (
  ...
  const handleDeleteBill = async (bill: BillAccount) => {
    ...
    try {
        ...
        setBills((prev) =>
          prev.map((currentBill) =>
            currentBill.id === bill.id
              ? { ...currentBill, hiddenFromBills: true }
              : currentBill,
          ),
        );
        return;
      ...
      setBills((prev) =>
        prev.filter((currentBill) => currentBill.id !== bill.id),
      );
    } catch (error) {
  ...
  const handleDeleteMonthlyBill = async (scope: "month" | "series") => {
    if (!billToDelete?.recurringSeriesId) return;

    setIsProcessing(true);
    try {
      if (scope === "series") {
        await deleteMonthlyBillAccountSeries(billToDelete.recurringSeriesId);
        setBills((prev) =>
          prev.filter(
            (bill) => bill.recurringSeriesId !== billToDelete.recurringSeriesId,
          ),
        );
      } else {
        await deleteMonthlyBillAccountOccurrence(billToDelete.id);
  ...
  const handleUnpayBill = async (billId: string) => {
    ...
    if (legacyPayment) {
      const transactionDate = new Date(
        `${legacyPayment.date}T12:00:00`,
      ).toLocaleDateString("pt-BR");
      const similarTransactionsMessage =
        legacyPaymentCandidates.length > 1
    ...
    try {
      ...
      const [updatedBills, updatedTransactions, updatedCards] =
        await Promise.all([
          getBillAccounts(),
          getTransaction(),
          getUserCreditCards(),
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6">
          ...
          <div className="grid gap-4 py-4">
            <div className="relative">
              <Input
                ...
                onBlur={() =>
                  setTimeout(() => setIsDescriptionFocused(false), 150)
                }
              />
              {isDescriptionFocused &&
                getCardsWithOpenInvoices().length > 0 && (
                  <div className="absolute top-full left-0 right-0 z-10 mt-1 rounded-lg border border-border/60 bg-white shadow-lg dark:bg-slate-900">
                    {getCardsWithOpenInvoices().map(
                      ({ card, invoiceAmount, periodKey, dueDate }) => (
                        <button
                          key={`${card.id}-${periodKey}`}
                          onClick={() =>
                            handleSelectCardSuggestion(
                              card,
                              invoiceAmount,
                              periodKey,
                              dueDate,
                        ...
                        >
                          <p className="text-sm text-[#0F172A] dark:text-white">
                            Fatura do cartão{" "}
                            {BANKS[card.bankKey as BankKey]?.name ||
                              card.bankKey}
                          </p>
                        </button>
                      ),
        ...
        <DialogContent className="max-sm:max-w-106 max-md:max-w-xl lg:max-w-202">
          ...
          <div className="grid gap-4 py-4">
            <p className="text-sm text-[#64748B] dark:text-[#94A3BB]">
              Deseja adicionar o pagamento desta conta automaticamente à sua
              lista de transações?
            </p>
            {selectedBillForPayment && (
              <div className="rounded-2xl border border-border/60 bg-[#F8FAFC] px-4 py-4 dark:bg-white/5">
                <p className="text-sm font-medium text-[#0F172A] dark:text-white">
                  {selectedBillForPayment.description}
                </p>
      ...
      >
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            <DialogTitle>Remover conta fixa</DialogTitle>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            <p className="text-sm text-[#64748B] dark:text-[#94A3BB]">
              {billToDelete
                ? `“${billToDelete.description}” é uma conta fixa mensal. Escolha o que deseja remover.`
                : ""}
            </p>
          </div>
          <DialogFooter>
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => handleDeleteMonthlyBill("month")}
              disabled={isProcessing}
              className="w-full sm:w-auto"
            >
              Apenas este mês
            ...
            <Button
              type="button"
              onClick={() => handleDeleteMonthlyBill("series")}
              disabled={isProcessing}
              className="w-full sm:w-auto"
            >
              Todos os meses
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-6 text-[#64748B] dark:text-[#94A3BB]">
              Acompanhe suas contas a pagar, visualize seu saldo projetado e
              registre novos compromissos financeiros.
            </p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            variant={periodFilter === "next" ? "default" : "outline"}
            onClick={() => setPeriodFilter("next")}
            size="sm"
            className="rounded-full"
          >
            Próximo Mês
                ...
                >
                  ...
                  <div className="flex gap-3 max-sm:gap-0.5 items-end max-sm:flex-col">
                    <div className="text-right">
                      ...
                      <span
                        className={`inline-flex rounded-full border px-2 max-sm:px-1 py-1 text-xs font-semibold ${statusColor}`}
                      >
                        {statusLabel}
                      </span>
                    </div>
