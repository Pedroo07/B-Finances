...
import {
  TransactionHeader,
  TransactionItem,
} from "@/app/dashboard/components/transactions";
import {
  DonutChart,
  GraphicListItem,
  separateAmountByCategory,
} from "@/app/dashboard/components/graphic";
...
import {
  ...
  updateUserCreditCardBilling,
} from "@/lib/services/userCreditCards";
...
export const Main = () => {
  ...
  const visibleAddedCardKeys = visibleUserCards.reduce<BankKey[]>(
    (currentKeys, currentCard) => {
  ...
  const availableCardKeys = (Object.keys(BANKS) as BankKey[]).filter(
    (bankKey) => !visibleAddedCardKeys.includes(bankKey),
  );

  const effectiveSelectedCardKey =
    selectedCardKey && cards.some(([bankKey]) => bankKey === selectedCardKey)
      ? selectedCardKey
      : (cards[0]?.[0] ?? null);
  ...
  const currentSelection =
    selectedCardIndex >= 0 ? cards[selectedCardIndex] : (cards[0] ?? null);
  const currentCard = currentSelection?.[1] ?? null;
  const currentUserCard = effectiveSelectedCardKey
    ? (visibleUserCards.find(
        ({ bankKey }) => bankKey === effectiveSelectedCardKey,
      ) ?? null)
    : null;
  ...
  const currentCardClosingDay = hasCurrentCardBilling
    ? currentUserCard!.closingDay!
    : null;
  const currentCardDueDay = hasCurrentCardBilling
    ? currentUserCard!.dueDay!
    : null;
  const selectedTransactionCard = cards.some(
    ([, currentBank]) => currentBank.name === card,
  )
    ? card
    : (currentCard?.name ?? "");
  ...
  const canSaveNewCard =
    Boolean(cardToAdd) &&
    isValidBillingDay(closingDayToAdd) &&
    isValidBillingDay(dueDayToAdd);
  const canSaveBilling =
    Boolean(effectiveSelectedCardKey) &&
    isValidBillingDay(billingClosingDay) &&
    isValidBillingDay(billingDueDay);
  ...
  const handleInvoicePaymentDateChange = (
    event: ChangeEvent<HTMLInputElement>,
  ): void => {
    const newValue = event.target.value;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const validUserCards = userCards.filter(({ bankKey }) =>
          isBankKey(bankKey),
        );
  ...
  const currentCardItems = currentCard
    ? visibleItems.filter(
        (transaction) => transaction.card === currentCard.name,
      )
    : [];
  const currentInvoicePeriodKey =
    activeFilter === "month" && selectedMonth > 0
      ? getInvoicePeriodKey(currentYear, selectedMonth)
      : null;
  const currentInvoiceRange =
    currentInvoicePeriodKey && currentCardClosingDay && currentCardDueDay
      ? getInvoiceDateRange(
          currentInvoicePeriodKey,
          currentCardClosingDay,
          currentCardDueDay,
        )
      : null;
  ...
  const isInvoicePaid =
    Boolean(currentInvoicePeriodKey) &&
    hasCurrentCardBilling &&
    currentInvoiceAmount > 0 &&
    invoicePaidAmount >= currentInvoiceAmount;
  const canPayInvoice =
    Boolean(currentCard) &&
    hasCurrentCardBilling &&
    Boolean(currentInvoicePeriodKey) &&
  ...
  const handleAddNewItem = async () => {
    ...
    try {
      if (installmentCount === 1) {
        ...
        setItems((currentItems) =>
          sortItemByDate([...newTransactions, ...currentItems]),
        );
      }
  ...
  const handleDeleteSelectedInstallment = async () => {
    ...
    try {
      ...
      setItems((currentItems) =>
        currentItems.filter((item) => item.id !== installmentToDelete.id),
      );
  ...
  const handleDeleteInstallmentGroup = async () => {
    if (!installmentToDelete?.installmentGroupId || isDeletingInstallment)
      return;
    ...
    try {
      ...
      setItems((currentItems) =>
        currentItems.filter((item) => item.installmentGroupId !== groupId),
      );
      setInstallmentToDelete(null);
    } finally {
      setIsDeletingInstallment(false);
  ...
  const handleAddNewCard = async () => {
    ...
    try {
      ...
      setUserCards((currentCards) =>
        currentCards.some((currentCard) => currentCard.bankKey === cardToAdd)
          ? currentCards
  ...
  const handleSaveCurrentCardBilling = async () => {
    ...
    try {
      ...
      const updatedCard = await updateUserCreditCardBilling(
        effectiveSelectedCardKey,
        {
  ...
  const handleRemoveCurrentCard = async () => {
    ...
    try {
      ...
      setUserCards((currentCards) =>
        currentCards.filter((currentCard) => currentCard.bankKey !== bankKey),
      );
  ...
  const differenceInPorcentage = () => {
    ...
    const [selectedYear, selectedInvoiceMonth] = currentInvoicePeriodKey
      .split("-")
      .map(Number);
    const previousInvoiceYear =
      selectedInvoiceMonth === 1 ? selectedYear - 1 : selectedYear;
    const previousInvoiceMonth =
      selectedInvoiceMonth === 1 ? 12 : selectedInvoiceMonth - 1;
    const previousInvoicePeriodKey = getInvoicePeriodKey(
      previousInvoiceYear,
      previousInvoiceMonth,
    );

    const filteredItems = currentCardItems.filter(
      (item) =>
        getInvoicePeriodKeyForDate(
          item.date,
          currentCardClosingDay,
    ...
    const lastItems = currentCardItems.filter(
      (item) =>
        getInvoicePeriodKeyForDate(
          item.date,
          currentCardClosingDay,
          currentCardDueDay,
        ) === previousInvoicePeriodKey,
    );

    const totalExpense = Math.abs(
      filteredItems.reduce((acc, item) => acc + item.amount, 0),
    );
    const lastExpense = Math.abs(
      lastItems.reduce((acc, item) => acc + item.amount, 0),
    );
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6">
      <Dialog open={isAddCardDialogOpen} onOpenChange={setIsAddCardDialogOpen}>
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            ...
            <DialogDescription>
              Escolha um dos cartões disponiveis para liberar o acompanhamento
              dele nesta tela.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            >
              <span>
                {isSavingCard ? "Adicionando..." : "Adicionar cartao"}
              </span>
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        open={isRemoveCardDialogOpen}
        onOpenChange={setIsRemoveCardDialogOpen}
      >
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            <DialogTitle>Remover cartão?</DialogTitle>
            <DialogDescription>
              Tem certeza que deseja remover o cartão {currentCard?.name}? As
              transações dele nao serão apagadas, mas ele deixará de aparecer
              aqui ate ser adicionado novamente.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={isBillingDialogOpen} onOpenChange={setIsBillingDialogOpen}>
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            ...
            <DialogDescription>
              Informe os dias usados pelo banco para calcular as proximas
              faturas deste cartao.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            >
              <span>
                {isSavingBilling ? "Salvando..." : "Salvar configuracao"}
              </span>
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        ...
        onOpenChange={setIsPayInvoiceDialogOpen}
      >
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            <DialogTitle>Pagar fatura</DialogTitle>
            <DialogDescription>
              Marque sua fatura como paga, e adicione a despesa automaticamente
              a suas transações.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="rounded-2xl border border-border/60 bg-[#F8FAFC] px-4 py-4 dark:bg-white/5">
              ...
              {currentInvoiceRange && currentInvoiceDueDate ? (
                <p className="mt-2 text-sm text-[#64748B] dark:text-[#94A3BB]">
                  {formatDisplayDate(currentInvoiceRange.startDate)} a{" "}
                  {formatDisplayDate(currentInvoiceRange.endDate)}. Vence em{" "}
                  {formatDisplayDate(currentInvoiceDueDate)}.
                </p>
              ) : null}
            </div>
            <Input
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !invoicePaymentDate || !canPayInvoice || isPayingInvoice
              }
              className="w-full sm:w-auto"
            >
              <span>
                {isPayingInvoice ? "Pagando..." : "Confirmar pagamento"}
              </span>
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      >
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            ...
            <DialogDescription>
              Informe o valor total da compra. A data será usada como a primeira
              parcela.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            {expenseFormError ? (
              <p className="text-sm text-rose-500">{expenseFormError}</p>
            ) : null}
          </div>
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent className="sm:max-w-106">
          <DialogHeader>
            ...
            <DialogDescription>
              Exclua somente a parcela {installmentToDelete?.installmentNumber}/
              {installmentToDelete?.installmentCount} ou todas as parcelas
              cadastradas desta compra.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button
              type="button"
              variant="outline"
              onClick={handleDeleteSelectedInstallment}
              disabled={isDeletingInstallment}
            >
            ...
            <Button
              type="button"
              variant="destructive"
              onClick={handleDeleteInstallmentGroup}
              disabled={isDeletingInstallment}
            >
              Todas as parcelas
            </Button>
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-6 text-[#64748B] dark:text-[#94A3BB]">
              Acompanhe gastos do seu cartão e compare os resultados mês a mês
              em uma visualização mais limpa.
            </p>
          </div>
          <div className="flex flex-col gap-3 xl:items-end">
            ...
            {hasCards ? (
              ...
              <p className="max-w-md text-sm text-[#64748B] dark:text-[#94A3BB]">
                Adicione pelo menos um cartão para acompanhar seus gastos nessa
                pagina.
              </p>
            )}
          </div>
      ...
      {isPageLoading ? (
        ...
        <section className="surface-card flex flex-col items-start gap-4 p-6 sm:p-8">
          ...
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-6 text-[#64748B] dark:text-[#94A3BB]">
              Antes de aparecer qualquer cartão, você precisa adicionar pelo
              menos uma opção disponivel. Depois disso, somente os cartões
              adicionados ficam visiveis por aqui.
            </p>
          </div>
          <Button
            onClick={() => setIsAddCardDialogOpen(true)}
          ...
          <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="surface-card p-6">
              <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                <div className="space-y-2">
                  <p className="text-sm uppercase tracking-[0.28em] text-[#94A3BB]">
                    Cartao selecionado
                  </p>
                  <h2 className="text-2xl font-semibold text-[#0F172A] dark:text-white">
                    {currentCard?.name}
                ...
                <div className="flex items-center justify-center gap-4 rounded-[28px] border border-border/60 bg-white/50 px-4 py-4 backdrop-blur-sm dark:bg-white/5">
                  ...
                  {currentCard && (
                    <Image
                      ...
                      width={140}
                      height={140}
                      className="h-auto w-27 -rotate-90 sm:w-32"
                    />
                  )}
                  <button
                    className="flex h-11 w-11 items-center justify-center rounded-full border border-border/60 text-[#334155] transition-all hover:border-[#22C55E]/40 hover:text-[#22C55E] disabled:cursor-not-allowed disabled:opacity-50 dark:text-[#CBD5E1]"
            ...
            <div className="surface-card flex flex-col justify-between gap-5 p-6">
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span
                    className={`inline-flex rounded-full border px-3 py-2 text-sm font-semibold ${invoiceStatusClass}`}
                  >
                    {invoiceStatusLabel}
                  </span>
          ...
          <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
            <div className="surface-card flex items-center gap-4 p-5 sm:p-6">
              ...
              <div className="px-1">
                ...
                <p className="text-sm text-[#64748B] dark:text-[#94A3BB]">
                  Registre uma nova compra no cartão selecionado.
                </p>
              </div>
            </div>
            <section className="surface-card p-6">
              ...
              <p className="mt-1 text-sm text-[#64748B] dark:text-[#94A3BB]">
                Veja em quais categorias este cartão esta sendo mais utilizado.
              </p>
          ...
          <section className="surface-card-strong overflow-hidden">
            <header className="border-b soft-divider px-5 py-5 sm:px-6">
              <h4 className="text-xl font-semibold text-[#0F172A] dark:text-white">
                Ultimas transações do cartão
              </h4>
              <p className="text-sm text-[#64748B] dark:text-[#94A3BB]">
                Acompanhe os lançamentos recentes do cartão selecionado.
              </p>
            </header>
            <main>
              <TransactionHeader />
              <div className="max-h-96 overflow-auto">
                {filterItems.length > 0 ? (
                  ...
                  <p className="px-5 py-8 text-sm text-[#64748B] dark:text-[#94A3BB] sm:px-6">
                    Nenhuma transação encontrada para este cartão no periodo
                    selecionado.
                  </p>
                )}
              </div>
