...
const separateAmountByCategoryAndType = (
  ...
  currentItems.forEach((item) => {
    ...
    totalsByCategory[item.category] =
      (totalsByCategory[item.category] ?? 0) + absoluteAmount;
...
const separateCardAmountByCategory = (
  currentItems: CardTransaction[],
): CategoryResults => {
  const totalsByCategory: Record<string, number> = {};
  let totalAmount = 0;
...
export const Main = () => {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        const [transactionsData, cardTransactionsData, userCardsData] =
          await Promise.all([
            getTransaction(),
            getCardTransaction(),
            getUserCreditCards(),
        ...
        if (isMounted) {
          ...
          setUserCards(
            userCardsData.filter(({ bankKey }) => isBankKey(bankKey)),
          );
        }
      } catch (error) {
  ...
  const paymentFilteredItems = typeFilteredItems.filter((item) => {
    if (paymentMethodFilter === "all" || paymentMethodFilter === "credit_card")
      return true;
  ...
  const availableCategories = Array.from(
    ...
    translateCategory(a).localeCompare(translateCategory(b), "pt-BR"),
  );
        ...
        <div className="surface-card p-6">
          <div className="space-y-2">
            ...
            <p className="text-sm leading-6 text-[#64748B] dark:text-[#94A3BB]">
              Filtre seu historico e acompanhe a distribuição das transações por
              categoria.
            </p>
          </div>
          ...
          <div className="mt-6 space-y-3">
            <div className="grid gap-2 md:grid-cols-2">
              ...
              {isCreditCardMode ? (
                <Select
                  ...
                  onValueChange={setSelectedCardFilter}
                >
                  <SelectTrigger className="w-full">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <p className="text-xs leading-5 text-[#64748B] dark:text-[#94A3BB]">
                Ao preencher as datas, o intervalo abaixo substitui os atalhos
                acima.
              </p>
            </div>
          </div>
              ...
              <ul className="w-full divide-y divide-border/40">
                {loading ? (
                  ...
                  <li className="px-5 py-8 text-center text-sm text-[#64748B] dark:text-[#94A3BB] sm:px-6">
                    Nenhuma transação de cartão encontrada com os filtros
                    selecionados.
                  </li>
                )}
              </ul>
