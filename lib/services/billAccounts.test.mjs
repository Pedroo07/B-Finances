...
function documentReference(parent, path, id) {
  const fullPath =
    parent?.kind === "collection"
      ? `${parent.path}/${path ?? `auto-${++generatedDocumentCount}`}`
      : id === undefined
        ? path
        : `${path}/${id}`;
...
function commitOperations(operations) {
  for (const operation of operations) {
    if (operation.type === "delete") {
      ...
      applySet(operation.reference, operation.values, operation.options);
    } else {
...
const firestoreApi = {
  ...
  async getDocs(reference) {
    const collectionReference =
      reference.kind === "query" ? reference.collection : reference;
...
const typeStub = dataModule(`
  ...
  export const MonthlyBillAccountSeries = undefined;
...
const monthlySeriesPath = (id) => `users/user-1/billAccountRecurrences/${id}`;

test("monthly series creates same-day occurrences and preserves a skipped month", async () => {
  resetDocuments([]);

  const firstBill = await createMonthlyBillAccountSeries({
  ...
  const seriesId = firstBill.recurringSeriesId;

  assert.ok(seriesId);
  assert.equal(firstBill.dueDate, "2026-01-31");
  assert.deepEqual(
    generatedBills.map((bill) => bill.dueDate),
    ["2026-02-28", "2026-03-31"],
  );
  assert.equal(getMonthlyOccurrenceDueDate(31, "2028-02"), "2028-02-29");

  const februaryBill = generatedBills[0];
  await deleteMonthlyBillAccountOccurrence(februaryBill.id);
  await syncMonthlyBillAccountOccurrences(new Date(2026, 1, 20));

  assert.equal(firestoreState.documents.has(billPath(februaryBill.id)), false);
  assert.deepEqual(
    firestoreState.documents.get(monthlySeriesPath(seriesId)).excludedMonths,
    ["2026-02"],
  );

  await deleteMonthlyBillAccountSeries(seriesId);

  assert.equal(
    firestoreState.documents.has(monthlySeriesPath(seriesId)),
    false,
  );
  assert.equal(
    [...firestoreState.documents.keys()].some((path) =>
      path.startsWith("users/user-1/billAccounts/monthly_"),
    ),
    false,
  );
});
...
test("legacy ordinary payment is strictly ranked, revalidated, and deletes only the confirmed candidate", async () => {
  ...
  assert.deepEqual(
    getLegacyBillPaymentCandidates(legacyBill, rankedTransactions).map(
      ({ id }) => id,
    ),
    ["legacy-confirmed", "legacy-identical", "one-day-after", "one-day-before"],
  );
