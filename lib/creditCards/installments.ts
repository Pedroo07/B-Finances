...
function parsePurchaseDate(value: string): {
  ...
  if (!match)
    throw new Error("A data da compra deve estar no formato AAAA-MM-DD");
...
function getInstallmentDate(
  ...
  const targetMonth = new Date(
    Date.UTC(purchaseDate.year, purchaseDate.month - 1 + monthOffset, 1),
  );
...
export function buildInstallmentSchedule({
  ...
  if (
    ...
    installmentCount < 2 ||
    installmentCount > MAX_CARD_INSTALLMENTS
  ) {
    throw new Error(
      `A quantidade de parcelas deve estar entre 2 e ${MAX_CARD_INSTALLMENTS}`,
    );
  }
  ...
  if (totalCents < installmentCount) {
    throw new Error(
      "O valor total deve permitir parcelas de pelo menos R$ 0,01",
    );
  }
  ...
  const finalInstallmentCents =
    baseInstallmentCents + (totalCents % installmentCount);
