...
function formatDateParts({
  ...
  month,
  day,
}: InvoicePeriodParts & { day: number }): string {
...
function getPeriodKeyForDueDateParts(
  year: number,
  month: number,
  day: number,
): string {
...
function getInvoicePeriodKeyForClosingMonth(
  ...
  const dueDate = getDueDateAfterClosingDate(
    getClosingDate(year, month, closingDay),
    dueDay,
  );
  return getPeriodKeyForDueDateParts(dueDate.year, dueDate.month, dueDate.day);
}

export function getInvoicePeriodKeyForDate(
  date: string,
  closingDay: number,
  dueDay: number,
): string {
...
export function getInvoiceDateRange(
  ...
  const closingPeriod = [-1, 0, 1]
    .map((offset) => addMonths(year, month, offset))
    .find(
      (candidate) =>
        getInvoicePeriodKeyForClosingMonth(
          candidate.year,
          candidate.month,
          closingDay,
          dueDay,
  ...
  const previousClosingMonth = addMonths(
    closingPeriod.year,
    closingPeriod.month,
    -1,
  );
  const previousClosingDay = getClampedBillingDay(
...
export function getInvoicePeriodKeyForDueDate(
  dueDate: string,
  closingDay: number,
  dueDay: number,
): string {
