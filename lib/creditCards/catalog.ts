...
export const CREDIT_CARD_BANK_KEYS = Object.keys(
  CREDIT_CARD_CATALOG,
) as BankKey[];
export const CREDIT_CARD_NAMES: CreditCardName[] = CREDIT_CARD_BANK_KEYS.map(
  (bankKey) => CREDIT_CARD_CATALOG[bankKey].name,
);
...
function textContainsCardPhrase(
...
export function getCreditCardBankKey(value?: string | null): BankKey | null {
  ...
  const match = (Object.keys(CREDIT_CARD_CATALOG) as BankKey[]).find(
    (bankKey) => {
      const bank = CREDIT_CARD_CATALOG[bankKey];
      return (
        normalizeCardText(bankKey) === normalizedValue ||
        normalizeCardText(bank.name) === normalizedValue
      );
    },
  );
...
export function findCreditCardBankKeyInText(
  value?: string | null,
): BankKey | null {
  if (!value) return null;
  ...
  const match = CREDIT_CARD_BANK_KEYS.find((bankKey) => {
    return getCardTextAliases(bankKey).some((alias) =>
      textContainsCardPhrase(normalizedText, alias),
    );
  });
...
export function findCreditCardNameInText(
  value?: string | null,
): CreditCardName | null {
