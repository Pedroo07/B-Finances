...
const stubs = new Map([
  ...
  [
    "@/lib/services/admin/billAccountsAdmin",
    dataModule(`
    const state = globalThis.__financialQueryState;
  ...
  [
    "@/lib/services/admin/transactionsAdmin",
    dataModule(`
    const state = globalThis.__financialQueryState;
  ...
  [
    "@/lib/services/admin/userCreditCardsAdmin",
    dataModule(`
    const state = globalThis.__financialQueryState;
    ...
    export async function getUserCreditCards() { return state.userCards; }
  `),
  ],
  [
    "@/lib/services/admin/investmentsAdmin",
    dataModule(`
    const state = globalThis.__financialQueryState;
  ...
  [
    "../handlers/updateTransactionFlow",
    dataModule(`
...
test("resumo usa o mes completo e separa cartoes, categorias e contas", async () => {
  state.normalTransactions = [
    ...
    {
      id: "paid-inter",
      description: "Fatura do cartão Inter",
      date: "2026-07-20",
      amount: -500,
      category: "Credit Card",
      type: "expense",
  ...
  state.userCards = [
    {
      ...
      invoices: {
        "2026-07": {
          amountPaid: 700,
          paidAt: "2026-07-20",
          transactionId: "paid-nubank",
        },
        "2026-08": {
          amountPaid: 0,
          paidAt: "2026-08-20",
          transactionId: "open-nubank",
        },
      },
    },
    {
      ...
      invoices: {
        "2026-07": {
          amountPaid: 500,
          paidAt: "2026-07-20",
          transactionId: "paid-inter",
        },
      },
    },
    {
      id: "mercado_pago",
      bankKey: "mercado_pago",
      invoices: {},
    },
  ];
  ...
  assert.equal(result.totals.normalExpense, 1300);
  assert.equal(result.totals.cardExpense, 350);
  assert.deepEqual(result.cardBreakdown, [
    { cardName: "Nubank", total: 700, count: 1 },
    { cardName: "Inter", total: 500, count: 1 },
  ]);
  assert.deepEqual(result.categoryBreakdown, [
    { label: "Credit Card", total: 1200, count: 2 },
    { label: "foods", total: 400, count: 2 },
    { label: "delivery", total: 50, count: 1 },
  ]);
  assert.deepEqual(
    result.pendingBills.map(({ id }) => id),
    ["rent"],
  );

  const response = formatBFinanceResponse(result);
  assert.match(response, /Receitas: R\$\s*1\.000,00/);
  assert.match(response, /Gastos: R\$\s*1\.300,00/);
  assert.match(response, /Gastos por cartão: R\$\s*1\.200,00/);
  assert.match(response, /Nubank: R\$\s*700,00/);
  assert.match(response, /Inter: R\$\s*500,00/);
  assert.doesNotMatch(response, /Mercado Pago: R\$\s*35,55/);
