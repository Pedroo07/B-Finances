...
test("reconhece parcelado e dividido por extenso", () => {
  assert.deepEqual(extractInstallmentMention("Dividi a compra em dez vezes"), {
    requested: true,
    count: 10,
  });
  assert.deepEqual(
...
test("usa a vista quando nao existe indicacao", () => {
  assert.deepEqual(extractInstallmentMention("Comprei um tenis de 1500"), {
    requested: false,
    count: 1,
  });
});
...
test("identifica pedido incompleto e quantidade fora do limite", () => {
  assert.deepEqual(extractInstallmentMention("Comprei parcelado"), {
    requested: true,
    count: null,
  });
  assert.deepEqual(extractInstallmentMention("Comprei em 15x"), {
    requested: true,
    count: 15,
  });
  assert.deepEqual(
